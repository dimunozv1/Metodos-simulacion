...
const double tau=0.55; //Para fluidos crea inestabilidades que sea igual a 0.5, pero sirve en el aso de las ondas
...
public:
  ...
  void Print(const char * NombreArchivo,double Ufan);
};  
...
void LatticeBoltzmann::ImposeFields(double Ufan){
  ...
  for(ix=0;ix<Lx;ix++){
    for(iy=0;iy<Ly;iy++){
      ...
      //fan
      if(ix==0){//El ventilador esta completamente a a  izq
	for(i=0;i<Q;i++){
	  n0=n(ix,iy,i);
	...
	for(i=0;i<Q;i++){
      ...
      //Un punto extra para que no sea tan simetrico
...
void LatticeBoltzmann::Advection(void){
 ...
 void LatticeBoltzmann::Print(const char * NameFile, double Ufan){
   ...
   for(ix=0;ix<Lx;ix+=5){//para no imprimir todas las celdas, asi no esta tan cluttered
    for(iy=0;iy<Ly;iy+=5){
      rho0=rho(ix,iy,true);
