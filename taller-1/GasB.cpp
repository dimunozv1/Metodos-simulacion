...
void Cuerpo::Dibujese(void){
  cout<<" , "<<r.x()<<"+"<<R<<"*cos(t),"<<r.y()<<"+"<<R<<"*sin(t) , "
      <<Getx()<< "+"<<R*cos(theta)/7.0<<"*t,"<<Gety()<<"+"<<R*sin(theta)/7.0<<"*t";
}
...
int main(void){
  ...
  //-----------------       x0,  y0,      Vx0, Vy0, theta0, omega0,double m0,double R0 
  ...
  for(ix=0;ix<Nx;ix++)
    for(iy=0;iy<Ny;iy++){
      ...
      //-----------------------(   x0,   y0,          Vx0,          Vy0, m0,R0)
      Molecula[Nx*iy+ix].Inicie((ix+1)*10, (ix+1)*10   ,   sqrt(V0),sqrt(V0), 0,0,m0,R0);//OJO
    }
