...
tuple<double, double>forces(int flag)
{ double Px,Py;
...
double Cm=2*sumy/(A*R*omega*Ufan0*1.0);
double Ca=sumx*2/(A*1.0*Ufan0);
double Re=Ufan0*R/eta;
if(flag==0){return make_tuple(Re,Ca);}//Imprimir los numeros del coeficiente de arrastre y reynolds
if(flag==1){return make_tuple(Re,Cm);} 
...
void Start(double rho0, double Ux0, double Uy0,double tau0)
{
int ix,iy,i,n0;
tau=tau0;
utau=1.0/tau;
...
fnew[n0]=um_utau*f[n0]+utau*f_eq(rho0,Ux0,Uy0,i);
  ...
  void fields(double Ufan, int flag)// impose conditions for the problem
...
double Fdrag=0.5*P*2*R*Ufan0;

double Magnus=-0.5*P*2*R*R*Ufan0*omega;

salida<<ix<<" "<<iy<<" "<<Ux0/Ufan*2<<" "<<Uy0/Ufan*2<<"\t"<<Fdrag<<"\t"<<Magnus<<"\n";
}
salida<<"\n";
}
salida.close();
...
if(flag==1){
 name="RevsCm.dat";}
ofstream salida(name);
for(double tau=0.51;tau<7;tau+=x){
//Start
if(tau==2.6){
  x=0.2;
}
Aire.Start(rho0,Ufan0,0,tau);
//Run
for(t=0;t<tmax;t++){
Aire.Collision();
 Aire.fields(Ufan0,flag);
Aire.advection();
}
Aire.Collision();
 Aire.fields(Ufan0,flag);
Aire.advection();
tie(C, Re)=Aire.forces(flag);
salida<<C<<"\t"<<Re<<endl;

//Print
Aire.Show("punto_de.dat",Ufan0);
}
salida.close();
return 0;
} 
