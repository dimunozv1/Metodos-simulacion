...
//---- declarar constantes ---
const double g=9.8,  Gamma=M_PI/9,Ca=0.5,p=1.224;
const int N=1;
...
void Colisionador::CalculeFuerzas(Cuerpo * Balon,double dt){
  int i,j; vector3D Fg,Fa; double V;
 ...
 for(i=0;i<N;i++){
   V=Balon[i].V.norm();
   Fa= Balon[i].V*V*Ca*p*A*-0.5;
   Balon[i].AdicioneFuerza(Fa,0);}
}
...
void InicieAnimacion(void){
  ...
  cout<<"set output 'Puntob.gif'"<<endl;
  cout<<"unset key"<<endl;
  cout<<"set xrange[-1:14]"<<endl;
  cout<<"set yrange[0:2.5]"<<endl;
  cout<<"set title \"Tiro parabolico con Fg y Fa\" "<<endl; 
