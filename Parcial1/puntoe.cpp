...
//---- declarar constantes ---
const double g=9.8, Gamma=M_PI/9,Ca=0.5,p=1.224,Cm=1.0,Beta=1.0e-4;
const int N=1;
...
void Colisionador::CalculeFuerzas(Cuerpo * Balon,double dt){
  int i,j; vector3D Fg,Fa,Fm,wvec,Vvec,taux,Faux,tvec; double Vnorm,zita;
taux.load(0,0,0);
Faux.load(0,0,0);
...
tvec= wvec*Vnorm*Beta*-1.0;
 Balon[i].AdicioneFuerza(Faux,tvec);}
